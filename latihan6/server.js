const jwt = require('jsonwebtoken'); // Memanggil library JWT yang tadi sudah diinstal lewat CMD agar bisa digunakan di dalam file server.js.
const express = require('express');
const app = express(); //Memanggil framework Express.js untuk mendirikan server API.

app.use(express.json()); //Fungsi dasar agar server bisa membaca input data berformat JSON yang dikirim oleh klien.
...
app.post('/api/login', (req, res) => { //Membuat rute khusus untuk menerima data login
    ...
    if (username === HARDCODED_USER.username && password === HARDCODED_USER.password) {
        ...
        // 4. Generate Token (Set expired dalam 1 jam agar lebih aman)
        const token = jwt.sign(payload, SECRET_KEY, { expiresIn: '1m' }); //fungsi ini menggabungkan Payload dengan SECRET_KEY untuk mencetak token. { expiresIn: '1h' } adalah fitur keamanan agar token akan hangus secara otomatis dalam 1 jam.
        ...
        return res.json({
            ...
            token: token
        }); //Server membungkus token yang baru dicetak tadi ke dalam format JSON, lalu memberikannya kepada klien. Tugas klien (seperti aplikasi atau Postman) adalah menyimpan string token ini baik-baik.
    } else {
...
app.post('/api/convert-suhu', verifyToken, (req, res) => {
    ...
    res.json({
        ...
        message: `Halo ${req.user.role} ${req.user.username}, konversi berhasil!`,
        data: {
